...
This repo is built on the above with the addition of Playwright integrated to provide an example of blocking LaunchDarkly's connection

## Playwright - Blocking LaunchDarkly's Connection

The fixture in the playwright (`tests/fixtures.js`) covers two examples:
   1. Blocking a connection to LaunchDarkly
   2. Connecting to LaunchDarkly

This can be used to test how an application would behave when it cannot make any connection to LaunchDarkly
when it is started.

One could also test for different behavior in each scenario, for example, when connected to LaunchDarkly
we expect the background color to be blue(`tests/ld-connected/feature-flags.spec.js`), but when not connected and the default flag values are used, we expect
our background to be red (`tests/ld-blocked/defaults.spec.js`)

One can also run any test against both fixtures, for example in either scenario, my app should return a 200 (`tests/app-loads.spec.js`)

Please note that if you want to run these tests, you must walk through the rest of the `README.md` below and ensure the flags are set accordingly in LaunchDarkly as well as the clientID configured for the environment one is working in.
...
3. Rename the `.env.example` to `.env` and insert your client-side SDK key:
   ```
   REACT_APP_LAUNCHDARKLY_CLIENT_KEY=<YOUR client-side SDK key>
   # Optional: Enable CloudFront proxy mode
   REACT_APP_USE_CLOUDFRONT_PROXY=true
   ```
4. Close your current terminal and start a new session to ensure the `.env` is picked up

### CloudFront Proxy Configuration (Optional)

This app supports using a CloudFront distribution as a proxy for LaunchDarkly API calls. This can provide:
- Lower latency through CloudFront's global edge locations
- Enhanced security and control over API traffic
- Compliance with corporate networking policies

**To enable CloudFront proxy:**

1. **Environment Variable (Recommended):** Add to your `.env` file:
   ```
   REACT_APP_USE_CLOUDFRONT_PROXY=true
   ```

2. **Code Configuration:** In `src/index.js`, change line 30:
   ```javascript
   const useCloudFrontProxy = true; // Enable CloudFront proxy
   ```

**Requirements:**
- CloudFront distribution must be configured to proxy LaunchDarkly endpoints
- Distribution domain: `dhmotfv99i6iw.cloudfront.net` (or update `cloudFrontDomain` in code)
- See [LaunchDarkly Proxy Mode Documentation](https://launchdarkly.com/docs/sdk/features/relay-proxy-configuration/proxy-mode) for setup details
